 ...
 *
 * DESCRIPTION: 
 *  This file contains common routines that are used by the various PCIE
 *  components.
 *
...
function createLibMake(device, makelibname,targetname, objectPath, useProfiling)
{
   var tooldir;
   var cmdprefix;
   var targetDir;
   ...
   switch(stringname)
   {
    ...
    if ((targetname.name == "A15F") || (targetname.name == "A9F") || (targetname.name == "A8F"))
    {
        ...
        libmakefile.writeLine("OBJDIR =  ./obj/obj_" +targetname.suffix +"/" + device.toString() + "/" + targetDir +"/obj" + benchSuffix); 
...
function buildLibrary (socName, isDmaSoc, isSoc, libOptions, libName, target, libFiles, useProfiling) 
{
    ...
    var makefilelib= createLibMake(socName, lib+".mk",target,objectPath,useProfiling);  
