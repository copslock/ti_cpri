/******************************************************************************
 * FILE PURPOSE: PCIE LLD example files.
 ******************************************************************************
 * FILE NAME: module.xs
 *
 * DESCRIPTION: 
 *  This file contains the module specification for PCIE LLD example files.
 *
...
function modBuild() 
{
    ...
    var mkFiles = libUtility.listAllFiles ("makefile", "example", true);
    for (var k = 0 ; k < mkFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = mkFiles[k];
