/******************************************************************************
 * FILE PURPOSE: IOLINK Driver Source Module specification file.
 ******************************************************************************
 * FILE NAME: module.xs
 *
 * DESCRIPTION: 
 *  This file contains the module specification for the IOLINK Driver
 *
...
/* List of all the IOLINK Driver Files */
var IOLINKSockLibFiles = [
    "src/IOLINK_drv.c",
    "src/v0/IOLINK_v0.c",
 ...
 **************************************************************************
 * DESCRIPTION   :
 *  The function is used to build the IOLINK LLD Driver and to add the core
...
function modBuild() 
{
    if (socs.length != 0)
    {
        ...
        {
            var targetFiles = IOLINKSockLibFiles.slice(); /* make copy */
            var libOptions = {
        ...
        for (var soc=0; soc < soc_names.length; soc++) 
        {
            ...
            { 
                var targetFiles_soc = IOLINKSockLibFiles.slice(); /* make copy */
