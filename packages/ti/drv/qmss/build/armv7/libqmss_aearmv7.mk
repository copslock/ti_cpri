...
ARMV7LIBDIR ?= ./lib
ARMV7OBJDIR ?= ./obj
ARMV7OBJDIR := $(ARMV7OBJDIR)/qmss/lib
ARMV7OBJDIR_SO := $(ARMV7OBJDIR)/qmss/lib_so
ARMV7BINDIR ?= ./bin
DEBUG_FLAG  ?= -O2
...
$(ARMV7OBJDIR)/qmss_device_$(DEVICE).$(OBJEXT): device/$(DEVICE)/src/qmss_device.c $(ARMV7OBJDIR)/.created
	-@echo compiling $< ... 
	@$(CC) -c $(CFLAGS) $(DEVICE_DEFINE) $(INCS)  $< -o $@

$(ARMV7OBJDIR_SO)/qmss_device_$(DEVICE).$(OBJEXT): device/$(DEVICE)/src/qmss_device.c $(ARMV7OBJDIR_SO)/.created
	-@echo compiling $< ... 
	@$(CC) -c $(CFLAGS) $(DEVICE_DEFINE) -fPIC $(INCS)  $< -o $@

$(ARMV7LIBDIR)/libqmss.a: $(COMMONSRCCOBJS) $(ARMV7LIBDIR)/.created
	@echo archiving $? into $@ ...
	@$(AR) -r $@ $?

$(ARMV7LIBDIR)/libqmss_$(DEVICE).a: $(COMMONSRCCOBJS) $(ARMV7OBJDIR)/qmss_device_$(DEVICE).$(OBJEXT) $(ARMV7LIBDIR)/.created
	@echo archiving $? into $@ ...
	@$(AR) -r $@ $?

libqmss.so: $(COMMONSRCCOBJS_SO)
...
libqmss_$(DEVICE).so: $(COMMONSRCCOBJS_SO) $(ARMV7OBJDIR_SO)/qmss_device_$(DEVICE).$(OBJEXT)
	@echo archiving $? into $(ARMV7LIBDIR)/$@.1 ...
	@$(CC) $(DEBUG_FLAG) -ggdb2 -Wl,-soname=libqmss_device.so.1 -shared -fPIC ${LDFLAGS} -o $@.1.0.0 $^
	@ln -s $@.1.0.0 $@.1
	@ln -s $@.1     $@
	@mv -f $@.1.0.0 $(ARMV7LIBDIR)/$@.1.0.0
	@mv -f $@.1 $(ARMV7LIBDIR)/$@.1
